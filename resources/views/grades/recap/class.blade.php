...
<div class="grid gap-5 lg:gap-7.5">
    ...
    <div class="kt-card"><div class="kt-card-content p-0 kt-scrollable-x-auto"><table class="kt-table table-auto kt-table-border"><thead><tr><th>Mahasiswa</th>@foreach($class->gradeComponents as $component)<th>{{ $component->name }}<br><span class="font-normal">{{ $component->weight }}%</span></th>@endforeach<th>Nilai Akhir</th></tr></thead><tbody>
    @forelse($class->members as $member)<tr><td>{{ $member->student->name }}@if(!$member->student->is_active)<span class="text-secondary-foreground"> (nonaktif)</span>@endif</td>@foreach($class->gradeComponents as $component)<td>{{ $component->scores->firstWhere('student_id', $member->student_id)?->score ?? '—' }}</td>@endforeach<td>{{ $class->finalGrades->firstWhere('student_id', $member->student_id)?->final_score ?? '—' }}</td></tr>@empty<tr><td colspan="{{ $class->gradeComponents->count() + 2 }}" class="py-8 text-center text-sm text-secondary-foreground">Belum ada mahasiswa.</td></tr>@endforelse
    </tbody></table></div></div>
</div>
@endsection
